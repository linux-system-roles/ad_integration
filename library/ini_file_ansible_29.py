...
# This is a vendored copy of community.general.ini_file, used only on
# ansible 2.9 (ansible-engine), where community.general cannot be installed.
# It carries no dependency on the collection so it can ship inside the role's
# library/ directory. Newer ansible uses the latest community.general.ini_file
# via the bare "ini_file" name instead. Keep in sync with upstream when needed.
